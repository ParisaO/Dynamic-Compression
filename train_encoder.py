...
if collect_dataset:
    dataset_path = os.path.join(os.getcwd(), "dataset")  # Absolute path: /Users/par/Desktop/new_1/dataset
    images_path = os.path.join(dataset_path, "images")   # Absolute path: /Users/par/Desktop/new_1/dataset/images"
    # Create directories if they don't exist
    os.makedirs(images_path, exist_ok=True)
    print(f"Collecting the dataset in: {dataset_path}")
    create_dataset(num_samples, dataset_path)
    print("Dataset collected")
...
if not retrain:
    ...
    if os.path.exists(models_path):
        ...
        encoder.load_state_dict(torch.load(os.path.join(models_path, 'encoder.pt')))
        # decoder.load_state_dict(torch.load('../models/decoder.pt'))
        decoder.load_state_dict(torch.load(os.path.join(models_path, 'decoder.pt')))
        encoder.eval()
        decoder.eval()
...
# Create a dataset
cvs_path = os.path.join(os.getcwd(), "dataset/description.csv")
root_dir = os.path.join(os.getcwd(), "dataset/images")
dataset = FramesDataset(cvs_path, root_dir, ToTensor())
...
for i in range(num_training_updates):
    for i_batch, sample_batched in enumerate(dataloader):
        ...
        if (i_batch+1) % 100 == 0:
            ...
            torch.save(quantizer.state_dict(), os.path.join(models_path, 'quantizer_' +
                                                            str(num_embeddings) + '_training.pt'))
            ...
            if retrain:
                ...
                torch.save(encoder.state_dict(), os.path.join(models_path, 'encoder_training.pt'))
                torch.save(decoder.state_dict(), os.path.join(models_path, 'decoder_training.pt'))

torch.save(quantizer.state_dict(), os.path.join(models_path, 'quantizer_' + str(num_embeddings)+'.pt'))
if retrain:
    # torch.save(encoder.state_dict(), '../models/encoder.pt')
    torch.save(encoder.state_dict(), os.path.join(models_path, 'encoder.pt'))
    # torch.save(decoder.state_dict(), '../models/decoder.pt')
    torch.save(decoder.state_dict(), os.path.join(models_path, 'decoder.pt'))