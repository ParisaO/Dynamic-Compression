...
encoder.load_state_dict(torch.load(os.path.join(models_path, 'encoder.pt'), map_location=torch.device('cpu')))
quantizer.load_state_dict(
    torch.load(os.path.join(models_path, 'quantizer_') + str(num_codewords) + '.pt', map_location=torch.device('cpu')))
decoder.load_state_dict(torch.load(os.path.join(models_path, 'decoder.pt'), map_location=torch.device('cpu')))
...
for i in num_quantization_levels[1:]:
    ...
    quantizer.load_state_dict(torch.load(os.path.join(models_path, 'quantizer_') + str(num_codewords_s[i - 1]) + '.pt',
...
model.load_state_dict(
    torch.load(os.path.join(models_path, 'policy_a2c_') + str(num_codewords) + '.pt', map_location=torch.device('cpu')))
...
if level == 'C':
    ...
    sensor_policy = sensor_3_levels(model, env, sensor_policy, list_of_quantizers, level, num_episodes, beta, encoder,
                                    gamma=0.99)
    torch.save(sensor_policy.state_dict(), os.path.join(models_path, 'sensor_level_') + level + '_a2c_' + str(beta) + '.pt')
...
if level == 'B':
    ...
    for i in num_quantization_levels[1:]:
        ...
        regressor.load_state_dict(
            torch.load(os.path.join(models_path, 'regressor_') + str(num_codewords_s[i - 1]) + '.pt', map_location=torch.device('cpu')))
    ...
    sensor_policy = sensor_2_levels(model, env, sensor_policy, list_of_quantizers, level, num_episodes, beta, encoder,
                                    list_of_regressors, gamma=0)
    torch.save(sensor_policy, os.path.join(models_path, 'sensor_level_') + level + '_a2c_' + str(beta) + '.pt')

# Level A
if level == 'A':
    from utils.lvs import sensor_1_levels

    sensor_policy = sensor_1_levels(model, env, sensor_policy, list_of_quantizers, level, num_episodes, beta, encoder,
                                    decoder, gamma=0)
    torch.save(sensor_policy.state_dict(), os.path.join(models_path, 'sensor_level_') + level + '_a2c_' + str(beta) + '.pt')
