...
agent_policy.load_state_dict(torch.load(os.path.join(models_path, "policy_a2c_64.pt"), map_location=torch.device('cpu')))
...
for i in num_quantization_levels[1:]:
    ...
    quantizer.load_state_dict(torch.load(os.path.join(models_path, 'quantizer_')+str(num_codewords_s[i-1])+'.pt', map_location=torch.device('cpu')))
    quantizer.eval()
...
for i in num_quantization_levels[1:]:
    ...
    regressor.load_state_dict(torch.load(os.path.join(models_path, 'regressor_')+str(num_codewords_s[i-1])+'.pt', map_location=torch.device('cpu')))
