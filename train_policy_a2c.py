### My code: python train_policy_a2c.py --num_episodes 10 --embedding_dim 64 --num_codewords 32
...
quantizer.load_state_dict(torch.load(os.path.join(models_path, 'quantizer_') + str(num_codewords)+'.pt', map_location=torch.device('cpu')))
...
torch.save(model.state_dict(), os.path.join(models_path, 'policy_a2c_')+str(num_codewords)+'.pt')