...
if retrain:
    ...
    name = os.path.join(models_path, 'sensor_level_A_a2c_0.1_train.pt')
    env = gym.make('CartPole-v1', render_mode='rgb_array')

    # sensor_policy.load_state_dict(torch.load(name, map_location=torch.device('cpu')).state_dict())
    sensor_policy.load_state_dict(torch.load(name, map_location=torch.device('cpu'), weights_only=False).state_dict())
    true_states = []
    #### It was 500 ###
...
for sample in range(len(sensor_actions)):
    if x[sample] > x_max or x[sample] < x_min:
        pass
    else:
        if theta[sample] > y_max or theta[sample] < y_min:
            pass
        else:
...
a = a * np.log(np.e * np.where(counter > 0, counter, 1))  # Replaces 0 with 1 to avoid log(0)
