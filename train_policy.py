### My command: python train_policy.py --num_codewords 64 --batch_size 64 --num_episodes 10
...
parser.add_argument('--exploring', type=float, help='The percentage to explore with an epsilon greedy policy (default is 0.5)', required=False)
...
quantizer.load_state_dict(torch.load(os.path.join(models_path, 'quantizer_') +str(num_codewords)+
...
torch.save(dqn.state_dict(), os.path.join(models_path, 'policy_') + str(num_codewords)+'.pt')